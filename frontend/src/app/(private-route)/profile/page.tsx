...
import { getAllProfileBlog } from "@/store/reducers/Profile/setProfileReducers";
import type { ProfileBlog } from "@/types/Profile/profile-types";
...
export default function Profile() {
	...
	const [cont, setCont] = useState<ProfileBlog[]>([]);
	useEffect(() => {
		dispatch(getAllProfileBlog())
			.unwrap()
			.then((response) => {
				setCont(response.blogPagesDto);
			})
			.catch(() => {
				toast.error("Failed to fetch profile blogs");
			});
	}, [dispatch]);
	return (
