...
export const ProfileEditPopOver = ({ profileData, onSave, children }: SharePopoverProps) => {
    ...
    const [socialLinks, setSocialLinks] = useState<SocialLink[]>(profileData.socialLinks);
    const [profileImage, setProfileImage] = useState<string>(profileData.profileImage);
    ...
    const predefinedTags = ['Developer', 'Designer', 'Writer', 'Photographer', 'Creator', 'Artist', 'Engineer', 'Entrepreneur', 'Student', 'Teacher', 'Manager', 'Freelancer'];
                ...
                <div className="flex flex-col space-y-8 max-h-[75vh] overflow-y-auto px-2 custom-scroll">
                        ...
                        <div className="flex flex-col md:flex-row gap-6 items-start">
                            ...
                            <div className="flex flex-col items-center gap-3">
                                <div
                                    ...
                                    onDrop={handleDrop}
                                >
                                    <div className={`w-36 h-36 rounded-2xl flex items-center justify-center overflow-hidden border-4 transition-all ${isDragging
                                        ? 'border-primary border-dashed scale-105 bg-primary/10'
                                        : 'border-border'
                                        }`}>
                                        {profileImage ? (
                                            <Image width={144} height={144} src={profileImage} alt="Profile" className={`w-full h-full object-cover transition-opacity ${isDragging ? 'opacity-50' : 'opacity-100'}`} />
                                        ) : (
                            ...
                            <div className="flex-1 w-full">
                                <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-4 lg:grid-cols-5 gap-3 max-h-[200px] overflow-y-auto pr-2 p-1 custom-scroll">
                                    {defaultProfileImages.map((img, index) => (
                                        <div
                                            key={index}
                                            className={`cursor-pointer rounded-xl overflow-hidden border-2 hover:border-primary transition-all duration-200 ${profileImage === img ? 'border-primary border-3' : 'border-gray-300 dark:border-gray-700'
                                                }`}
                                            onClick={() => selectDefaultImage(img)}
                                        >
                                            <Image width={60} height={60} src={img} alt={`Avatar ${index + 1}`} className="w-full h-full object-cover aspect-square" />
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="grid flex-1 gap-3">
                            ...
                            <span className="text-xs text-muted-foreground">Press Enter or comma to add. Start typing for suggestions.</span>
                        </div>
                    </div>
                    ...
                    <div className="grid gap-4 pt-6 border-t">
                        <div className="flex items-center justify-between">
                            <Label className="text-lg font-semibold">Social Links ({socialLinks.length}/5)</Label>
                            <Button
                                type="button"
                                size="sm"
                                variant="outline"
                        ...
                        <div className="space-y-3">
                            {socialLinks.map((link) => {
                                ...
                                return (
                                    <div key={link.id} className="flex items-end gap-3 p-4 border rounded-xl bg-muted/40 hover:bg-muted/60 transition-colors">
                                        <div className="grid flex-1 gap-3">
