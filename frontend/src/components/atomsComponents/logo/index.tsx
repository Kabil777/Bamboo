...
export function Logo({ logo = {
    ...
    return (
            <Link href={logo.url} legacyBehavior passHref className="flex items-center md:gap-1 transition-all delay-75">
                <div className={`${agbalumo.className} select-none cursor-pointer gap-1 md:gap-2 text-primary h-9 inline-flex w-max items-center justify-center rounded-md p-2 text-sm font-medium  disabled:opacity-50 transition-[color,box-shadow] focus-visible:ring-[3px] focus-visible:outline-1 pointer hover:bg-accent hover:text-accent-foreground`}>
