...
const DocsSlice = createSlice({
  ...
  reducers: {
    ...
    setPageContent: (state, action: PayloadAction<{ pageId: string; content: string }>) => {
    ...
    setSubPageContent: (state, action: PayloadAction<{ pageId: string; subPageId: string; content: string }>) => {
    ...
    updatePageTitle: (state, action: PayloadAction<{ pageIndex: number; title: string }>) => {
      const { pageIndex, title } = action.payload;
      if (state.Pages[pageIndex]) {
        state.Pages[pageIndex].title = title;
      }
    },
    updateSubPageTitle: (state, action: PayloadAction<{ pageIndex: number; subPageIndex: number; title: string }>) => {
    ...
    deleteSubItem: (state, action: PayloadAction<{ sectionIndex: number; subIndex: number }>) => {
...
export const { setAll, setTitleAndDescription, setContent, setPageContent, setSubPageContent, setPages, updatePageTitle, updateSubPageTitle, addMainSection, addSubItem, deleteMainSection, deleteSubItem } =
  DocsSlice.actions;
